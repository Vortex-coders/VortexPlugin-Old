...
public class Database {
    ...
    public static Mono<PlayerData> getPlayerData(String uuid) {
        return Mono
                .from(playersCollection.find(eq("uuid", uuid)))
                .defaultIfEmpty(new PlayerData(uuid));
    }
    ...
    public static Mono<BanData> getBan(String uuid, String ip) {
        return Mono
                .from(bansCollection.find(getBanFilter(uuid, ip)))
                .defaultIfEmpty(new BanData());
    }

    public static Mono<UpdateResult> setBan(BanData data) {
        return Mono.from(
                bansCollection.replaceOne(
                        getBanFilter(data.uuid, data.ip),
                        data,
                        new ReplaceOptions().upsert(true)
                )
        );
    ...
    public static Mono<DeleteResult> unBan(BanData data) {
        return Mono.from(
                bansCollection.deleteOne(getBanFilter(data.uuid, data.ip))
        );
    }

    private static Bson getBanFilter(String uuid, String ip) {
        return or(
                and(eq("uuid", uuid), eq("server", config.gamemode.name())),
