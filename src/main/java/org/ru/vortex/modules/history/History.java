...
public class History {
    ...
    public static double shortenedUUIDMaxValue =
            Math.pow(2, shortenedUUIDBites) - 1;
    public static CirculatingArray<byte[]> shortenedUUIDs = new CirculatingArray<>(
            new byte[(int) shortenedUUIDMaxValue][]
    );
    ...
    public static void init() {
        shortenedUUIDs.onRemove(index -> {
            ...
            long biteValueMask =
                    ((long) Math.pow(2, shortenedUUIDBites) - 1) << shift;
            ...
            IntStream
                    ...
                    .forEach(historyIndex -> {
                        if (
                                (history.get((int) historyIndex) & biteValueMask) >> shift ==
                                        nativeIndex
    ...
    public static void initHistory() {
        blocksIDBites =
                Pipe
                        ...
                        .result();
        xBites =
                Pipe.apply(Vars.world.width()).pipe(History::mathBitesSize).result();
        yBites =
                Pipe.apply(Vars.world.height()).pipe(History::mathBitesSize).result();
    ...
    public static Long compressHistory(FormattedEntry entry) {
        ...
        if (
                !shortenedUUIDs.contains(element -> Arrays.equals(element, decodedUUID))
        ) shortenedUUIDs.add(decodedUUID);

        int shortenedUUID = shortenedUUIDs.indexOf(element ->
                Arrays.equals(element, decodedUUID)
        );
    ...
    public static FormattedEntry decompressHistory(long compressedHistory) {
        ...
        Block block = Pipe
                .apply((int) packager.readBites(blocksIDBites))
        ...
        Administration.PlayerInfo playerInfo = Pipe
                ...
                .result()
                .first();
        BlockChangeType changeType = Pipe
                .apply((byte) packager.readBites(changeTypeBites))
                .pipe(BlockChangeType::getByID)
                .result();
        FormattedEntry historyEntry = changeType.formatEntry(
                playerInfo,
                block,
                position,
                additionalData
    ...
    public static byte additionalDataSize() {
        return (byte) (
                Long.BYTES *
                        8 -
                        (xBites + yBites + blocksIDBites + shortenedUUIDBites + changeTypeBites)
        );
    }
